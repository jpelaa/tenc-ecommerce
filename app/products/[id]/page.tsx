...
export default function Home(props) {
  ...
  const getProductDataFunc = async (id) => {
    setLoader(true);
  ...
  useEffect(() => {
    getProductDataFunc(props?.params?.id);
  }, [props?.params?.id]);
      ...
      <div className="w-full h-full py-6 px-5 bg-white">
        ...
        <div className="w-full h-full mt-3">
          <div className="flex flex-col flex-wrap justify-center items-center">
            <div className="h-60 w-60 flex items-center justify-center">
              {selectedProductData?.image ? (
                <Image
                  src={`/products/${selectedProductData.image}.png`}
                  alt="Menu"
          ...
          <div className="py-2 my-2">
            <p className="text-base">{selectedProductData?.name}</p>
            <p className="text-lg font-bold">₹ 699</p>
          </div>
          <div className="py-2 my-2 w-full flex justify-center items-center">
            {selectedProductData?.availableSizes.map((data) => {
              return (
                <div
                  onClick={() => setSelectedSize(data)}
                  key={data}
                  className={`border-[0.5px] border-gray-300 rounded-[50%] px-4 py-2 mx-3 ${
                    selectedSize === data
